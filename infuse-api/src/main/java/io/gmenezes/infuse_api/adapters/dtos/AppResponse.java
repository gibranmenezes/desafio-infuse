...
public class AppResponse<T> {
    ...
    public ResponseEntity<AppResponse<T>> getResponseEntity() {
        if (this.isValid()) {
            if (this.getContent() == null || ObjectUtils.isEmpty(content)) {
                this.status = 204;
            }
        } else {
            if (this.status < 400) {
    ...
    public static <T> AppResponse<T> invalid(String message, HttpStatus httpStatus, String errorCode, String descriptionError, String traceId) {
        return new AppResponse<>(httpStatus.value(),
                false,
                message,
                null,
                Collections.singletonList(AppErrorResponse.builder().code(errorCode).description(descriptionError).traceId(traceId).build()),
                null);
    }
    ...
    public static <T> AppResponse<T> badRequest(String message, String errorCode, String descriptionError, String traceId) {
        return AppResponse.invalid(message, HttpStatus.BAD_REQUEST, errorCode, descriptionError, traceId);
    }

    public static <T> AppResponse<T> notFound(String message, String errorCode, String descriptionError, String traceId) {
        return AppResponse.invalid(message, HttpStatus.NOT_FOUND, errorCode, descriptionError, traceId);
    }

    public static <T> AppResponse<T> internalServerError(String message, String errorCode, String descriptionError, String traceId) {
